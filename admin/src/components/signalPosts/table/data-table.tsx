...
import { DebouncedInput } from "../../globalSearch";
...
export function DataTable<TData, TValue>({
  ...
  const deleteSelected = async () => {
    ...
    const ids = selectedRows.map(
      (row) => (row.original as { _id: string })._id,
    );
    try {
      const url = import.meta.env.VITE_API_URL + "/signal";
      await axios.delete(url, { data: ids }).then(() => {
            ...
            {table.getRowModel().rows?.length ? (
              table.getRowModel().rows.map((row) => (
                <TableRow
                  ...
                  onClick={() =>
                    navigate("/listnettoyage", {
                      state: { signal: row.original },
                    })
                  }
                >
